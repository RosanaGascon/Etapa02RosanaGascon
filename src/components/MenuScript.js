...
const MenuScript = () => {
      ...
      .then(menuItems => {
        ...
        const validateForm = () => {
          ...
          if (!name || !address || !phone || !email) {
            window.Swal.fire("Por favor, completa todos los campos de contacto antes de calcular el total.");
            return false;
          }
          ...
          if (!hasOrder) {
            window.Swal.fire("Por favor, selecciona al menos un artículo antes de calcular el total.");
            return false;
          }
        ...
        const submitOrder = () => {
          ...
          // Corrigiendo la extracción del precio
          let total = orderDetails.reduce((acc, itemDetail) => {
            const priceMatch = itemDetail.match(/\$\d+(\.\d+)?/);
            if (priceMatch && priceMatch[0]) {
              acc += parseFloat(priceMatch[0].substring(1)); // Acceder al primer elemento del array
            }
            return acc;
          }, 0);
        ...
        // Inicializar los inputs cuando la ventana se carga
  ...
  return <OrderModal orderMessage={orderMessage} />;
};
